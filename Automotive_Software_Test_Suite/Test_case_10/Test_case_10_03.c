...
float32 Test_case_10_03_Func3(float32 floatValue) {
...
float32 Test_case_10_03_Func2(const uint8 *uint_ptr, size_t size_var) {
    float32 num = 0.0f;
    if ((uint_ptr != NULL) && ((size_var + sizeof(float32)) <= 1024u)) {

        num = Test_case_10_03_Func3(*(const float32*)&uint_ptr[size_var]);

    }
    return num;
}

float32 Test_case_10_03_Func1(const struc_2 *param_ptr, const uint32 param_uint) {
    float32 float_var = 0.0f;
    if ((param_ptr != NULL) && (param_ptr->uchar_ptr != NULL) && (param_uint <= (61u + 3u + 1u))) {
        float_var = Test_case_10_03_Func2(param_ptr->uchar_ptr, (((size_t)2 * sizeof(uint32))) + (param_ptr->uint_var * (size_t)3 * sizeof(float32)) + (sizeof(float32) * param_uint));
    }
...
int main_Test_case_10_05() {
    ...
    float32 temp = Test_case_10_03_Func1(struc_arr[0].struc_sub_var.struc_2_arr,1u);
    return 0;
}
